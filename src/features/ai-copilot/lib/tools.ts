...
export const tools = {
  search_products: {
    description: 'Search for beauty products by keywords or visual similarity. Use this to find products the user does not own.',
    parameters: z.object({
  ...
  analyze_collection: {
    ...
    execute: async ({ userId }: { userId: string }) => {
      ...
      const categories = [...new Set(ownedProducts.map(p => p.category).filter(Boolean))];
  ...
  create_makeup_look: {
    description: 'Generate a complete makeup look based on user context, preferences, and collection.',
    parameters: z.object({
      userId: z.string().describe('The ID of the user'),
      context: z.string().describe('Context for the look (e.g., "wedding", "office", "date night")'),
      preferences: z.string().optional().describe('Additional preferences (e.g., "natural", "bold")'),
    }),
    execute: async ({ userId, context, preferences }: { userId: string, context: string, preferences?: string }) => {
  ...
  calculate_budget: {
    ...
    execute: async ({ productIds }: { productIds: string[] }) => {
      ...
      return {
        ...
        details: products.map(p => ({ name: p.product_name, price: p.price })),
      };
    },
  },
};
