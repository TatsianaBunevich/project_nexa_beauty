...
import { CopilotMessage, CopilotResponse, CopilotRequest } from '@/types/copilot';
import {
  searchProducts, searchProductsTool
} from './tools/search-products';
import {
  analyzeCollection, analyzeCollectionTool
} from './tools/analyze-collection';
import {
  createMakeupLook, createMakeupLookTool
} from './tools/create-makeup-look';
import {
  findDupes, findDupesTool
} from './tools/find-dupes';
import {
  calculateBudget, calculateBudgetTool
} from './tools/calculate-budget';
import { z } from 'zod';
...
export class CopilotAgent {
  async run(request: CopilotRequest, history: CopilotMessage[] = []): Promise<CopilotResponse> {
    const { userId, message } = request;

    console.log(`[CopilotAgent] Starting run for user ${userId}`);

    // 1. Augment initial prompt with RAG context
    ...
    while (iteration < MAX_ITERATIONS) {
      ...
      if (messageContent.tool_calls) {
        ...
        try {
          ...
          console.warn('[CopilotAgent] Failed to parse final response as JSON, using fallback');
          parsedResponse = {
